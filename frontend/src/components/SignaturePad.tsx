...
interface Props {
  ...
  height?: number;
  label?: string;
  savedSignature?: string | null;         // user's profile signature, for quick "use saved"
}

/**
 * Reusable signature pad: draw with mouse/touch, clear, or load a saved signature.
...
const SignaturePad = ({ value, onChange, height = 150, label, savedSignature }: Props) => {
  ...
  const loadImage = (src: string) => {
    ...
    img.onload = () => {
      ...
      // Fit image within the canvas keeping aspect
