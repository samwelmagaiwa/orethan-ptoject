...
import { getOrgSettings, getCompanyName } from "./orgSettings";
...
async function buildA4(r: ReceiptData) {
  ...
  <div style="max-width:700px;margin:0 auto;color:#0f172a">
    ...
    <div style="margin-top:20px;text-align:center">
      ...
      <div style="font-size:10px;color:#94a3b8;margin-top:6px;font-style:italic">Generated By: ${getCompanyName()} · ${new Date().toLocaleString("en-GB")}</div>
    </div>
  </div>`;
...
async function buildThermal(r: ReceiptData, mm: 58 | 80) {
  ...
  const thermalLogo = org.company_logo_url || staticLogo;
  const thermalName = (org.company_name || "Orethan Microfinance").toUpperCase();
  const thermalAddr = org.company_address || ORETHAN_CONTACTS.box;
  const thermalEmail = org.company_email  || ORETHAN_CONTACTS.email;
  const thermalPhone = org.company_phone  || ORETHAN_CONTACTS.phone;
  ...
  const body = `
    <div style="text-align:center">
      <img src="${thermalLogo}" style="height:46px;object-fit:contain" alt="" />
      <div style="font-weight:800;font-size:13px;margin-top:3px">${thermalName}</div>
      <div style="font-size:9px;color:#333;line-height:1.35">${thermalAddr}<br/>${thermalEmail}<br/>${thermalPhone}</div>
    </div>
    ${hr}
    ...
    <div style="text-align:center;font-size:9px;color:#444">Generated By: ${getCompanyName()}</div>
