...
export const letterheadBlock = () => {
  ...
  const name     = org.company_name      || "Orethan Microfinance";
  const tagline  = org.company_tagline   || "";
  const address  = org.company_address   || ORETHAN_CONTACTS.box;
  const email    = org.company_email     || ORETHAN_CONTACTS.email;
  const phone    = org.company_phone     || ORETHAN_CONTACTS.phone;
  const brand    = org.brand_color       || "#1565c0";
  const regLine  = [
    org.company_registration_no && `Reg: ${org.company_registration_no}`,
    org.company_tin && `TIN: ${org.company_tin}`,
  ].filter(Boolean).join("  ·  ");

  return `
  <div style="display:flex;align-items:center;justify-content:space-between;gap:24px;flex-wrap:wrap">
    <div>
      <img src="${logoSrc}" style="height:88px;width:auto;object-fit:contain" alt="${name}" />
      <div style="font-size:18px;font-weight:900;color:#102a43;margin-top:4px;letter-spacing:-0.3px">${name}</div>
      ${tagline ? `<div style="font-size:11px;color:#64748b;margin-top:1px">${tagline}</div>` : ""}
      ${regLine ? `<div style="font-size:10px;color:#94a3b8;margin-top:2px">${regLine}</div>` : ""}
    </div>
    <div style="display:flex;flex-direction:column;gap:9px">
      ${address ? row(ICON_MAIL, address) : ""}
      ${email   ? row(ICON_AT,   email)   : ""}
      ${phone   ? row(ICON_PHONE, `<strong>Mobile:</strong> ${phone}`) : ""}
    </div>
  </div>
  <div style="display:flex;margin:14px 0 4px;height:7px;border-radius:4px;overflow:hidden">
    ...
    <span style="flex:1;background:linear-gradient(90deg,${brand},#1d8ad1)"></span>
  </div>`;
};
...
export const watermarkBlock = () => {
  ...
  <div style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-28deg);opacity:0.07;z-index:0;pointer-events:none">
    <img src="${logoSrc}" style="width:560px;max-width:80vw" alt="" />
  </div>`;
};

/**
 * Print only after all images have loaded. Guards against double print dialogs.
 */
...
/**
 * Open a print window with the dynamic letterhead + provided body HTML.
 */
export function printDocument(title: string, bodyHtml: string, ref?: string) {
  ...
  win.document.write(`<html><head><title>${title}</title><style>${baseStyles}</style></head><body>
    ...
    <div style="position:relative;z-index:1">
      ...
      ${bodyHtml}
      <div class="doc-footer">This is a system generated document. &copy; ${new Date().getFullYear()} ${companyName}. All rights reserved.</div>
    </div>
  </body></html>`);
  win.document.close();
  triggerPrint(win);
