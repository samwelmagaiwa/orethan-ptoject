...
class LoanController extends Controller
{
    ...
    // ADJUST LOAN AMOUNT/PERIOD (GM or MD only)
    public function adjust(Request $request, $id)
    {
        try {
            ...
            if (!in_array($role, ['general_manager', 'managing_director', 'admin'])) {
                return $this->error('Only GM, MD, or Admin can adjust loan amount', 403);
            }
            ...
            $mergedDetails = array_merge(
                is_array($loan->details) ? $loan->details : [],
                $data['details'] ?? [],
            ...
            Log::error('Loan adjust error: ' . $e->getMessage());
            return $this->error($e->getMessage(), 500);
        }
    }
